...
function getSheetInfo(values, rows) {
    ...
    for (var i = 0; i < values.length; i++) {
        ...
        var rowInfo = {
            row: i + 1,
            isInsert: (rows && (i + 1 >= rows.rowFirst && i + 1 <= rows.rowLast)),
            isItem: false,
            isExpense: false
        };
        ...
        switch (values[i][0]) {
            case 'Items':
                ...
                if (sheetInfo.isSummary) {
                    ...
                    if (!rows) ranges = ranges.concat(getLaborSectionGroupRanges(sheetInfo, rowInfo));
                }
                break;
            case 'Labor':
                section = 'Labor';
            ...
            default:
                {
                    if (section == 'Items') {
                        ...
                        ranges = ranges.concat(getItemSectionRanges(sheetInfo, rowInfo));
                    }
                        ...
                        if (!rows) {
                            ...
                            if (values[i][1].length) { // This is a labor group
                                ...
                                ranges = ranges.concat(getLaborSectionGroupRanges(sheetInfo, rowInfo));

                                sheetInfo['rowSum'] = i + 1;
                            }
                            else {

                                ranges = ranges.concat(getLaborSectionRanges(sheetInfo, rowInfo));
                            }
                        }
                    }
                    else if (section == 'Expenses') {

                        rowInfo.isExpense = true;

                        ranges = ranges.concat(getExpenseSectionRanges(sheetInfo, rowInfo));
                    }
                }
        }
    }
...
function getDefaultSectionRanges(sheetInfo, rowInfo) {
    ...
    // Input range formats7

    var inputRanges = [];

    if ((sheetInfo.isSummary && sheetInfo.isItem) || sheetInfo.isBom) inputRanges.push('A' + row);
